...
void CItem::DeleteCleanup(const bool fForce)
{
	...
	switch ( m_type )
	{
		...
		case IT_EQ_HORSE:
            ...
            if (CChar *pHorse = m_itFigurine.m_UID.CharFind(); pHorse && pHorse->IsDisconnected() && ! pHorse->m_pPlayer )
				{
...
CItem * CItem::GenerateScript( CChar * pSrc)
{
		...
		case IT_CONTAINER_LOCKED:
			{
                ...
                const auto pCont = dynamic_cast <CItemContainer *> ( this );
				ASSERT(pCont);
				pCont->MakeKey();
			}
			break;
    ...
    if (CResourceLock s; pItemDef->ResourceLock(s))
	{
...
CItem * CItem::CreateHeader( tchar * pArg, CObjBase * pCont, const bool fDupeCheck, CChar * pSrc )
{
	...
	const auto id = static_cast<ITEMID_TYPE>(iResIndex);
	...
	if ( pItem != nullptr )
	{
		...
		{
            const auto pContBase = dynamic_cast <CContainer *> ( pCont );
			ASSERT(pContBase);
    ...
    switch (GetType())
    {
        case IT_EQ_TRADE_WINDOW:
            ...
            if (!IsItemEquipped() || /*GetEquipLayer() != LAYER_NONE ||*/ !pChar || !pChar->m_pPlayer || !pChar->IsClientActive())
            {
                if (IsItemEquipped())
                {
                    if (const auto pCharCont = dynamic_cast<CChar *>(GetContainer()))
                    {
                        const auto pTradeCont = dynamic_cast<CItemContainer*>(this);
                        ASSERT(pTradeCont);
        ...
        {
            // Should not exist except equipped.
            if (const auto pItemTemp = dynamic_cast<CItemMemory *>(this); pItemTemp == nullptr)
            {
                iResultCode = 0x2222;
                return iResultCode;	// get rid of it.
            }
        ...
        case IT_BEARD:	// 62 = just for grouping purposes.
            ...
            if (!IsItemEquipped())
            {
                if (const auto pCont = dynamic_cast<CItemContainer *>(GetParent());
                    pCont == nullptr || (pCont->GetType() != IT_CORPSE && pCont->GetType() != IT_EQ_VENDOR_BOX))
                {
                    iResultCode = 0x2227;
                    return iResultCode;	// get rid of it.
                }
        ...
        default:
        {
            if (const IT_TYPE iType = GetType(); (iType > IT_QTY) && (iType < IT_TRIGGER))
            {
...
bool CItem::IsSameType( const CObjBase * pObj ) const
{
    ...
    const auto pItem = dynamic_cast <const CItem*> ( pObj );
	if ( pItem == nullptr )
		return false;
...
bool CItem::IsStackableException() const
{
	...
	// In a vendors box ?
    const auto pCont = dynamic_cast <const CItemContainer*>( GetParent());
	if ( pCont == nullptr )
		return false;
...
bool CItem::MoveToCheck( const CPointMap & pt, CChar * pCharMover )
{
	...
	if (pt.IsValidPoint())
    {
        ...
        {
            if (const CPointMap ptWall(CWorldMap::FindItemTypeNearby(pt, IT_WALL, 0, true, true)); !ptWall.IsValidPoint())
            {
                if (const CPointMap ptWindow(CWorldMap::FindItemTypeNearby(pt, IT_WINDOW, 0, true, true)); !ptWindow.IsValidPoint())
                {
                    ptNewPlace = pt;
                }
            }
	...
	if ( iDecayTime > 0 )
	{
        if (const CRegion *pRegion = ptNewPlace.GetRegion(REGION_TYPE_MULTI | REGION_TYPE_AREA | REGION_TYPE_ROOM);
            pRegion != nullptr && pRegion->IsFlag(REGION_FLAG_NODECAY) )
    ...
    if (IsTrigUsed(TRIGGER_DROPON_GROUND) || IsTrigUsed(TRIGGER_ITEMDROPON_GROUND))
    {
        ...
        if (const CPointMap ptChanged(ptcArgs); !ptChanged.IsValidPoint())
...
bool CItem::MoveNearObj( const CObjBaseTemplate* pObj, const ushort uiSteps )
{
	...
	ASSERT(pObj);
    if (const auto pPack = dynamic_cast<CItemContainer *>(pObj->GetParent()) )
	{
...
lpctstr CItem::GetName() const
{
	...
	else
	{
        ...
        if (const CObjBase *pObj = m_uidLink.ObjFind(); pObj && pObj != this )
		{
			return( pObj->GetName());
		}
		return "unnamed";
...
lpctstr CItem::GetNameFull(const bool fIdentified ) const
{
	...
	{
        if (const auto spell = static_cast<SPELL_TYPE>(ResGetIndex(m_itWeapon.m_spell)) )
		{
            if ( const CSpellDef *pSpellDef = g_Cfg.GetSpellDef(spell) )
			{
	...
	switch ( m_type )
	{
		case IT_LOOM:
			...
			{
                const auto AmmoID = static_cast<ITEMID_TYPE>(m_itLoom.m_ridCloth.GetResIndex());
                if ( const CItemBase *pAmmoDef = CItemBase::FindItemBase(AmmoID) )
		...
		case IT_ARCHERY_BUTTE:
			...
			{
                const auto AmmoID = static_cast<ITEMID_TYPE>(m_itArcheryButte.m_ridAmmoType.GetResIndex());
                if ( const CItemBase *pAmmoDef = CItemBase::FindItemBase(AmmoID) )
		...
		case IT_STONE_TOWN:
			{
                const auto pStone = dynamic_cast <const CItemStone*>(this);
				ASSERT(pStone);
		...
		case IT_BONE:
			if ( fIdentified )
			{
                if (const auto id = static_cast<CREID_TYPE>(ResGetIndex(m_itSkin.m_creid)))
				{
                    if (const CCharBase *pCharDef = CCharBase::FindCharBase(id); pCharDef != nullptr)
					{
	...
	if ( IsAttr(ATTR_STOLEN))
	{
        ...
        if ( const CChar *pChar = m_uidLink.CharFind() )
        {
...
height_t CItem::GetHeight() const
{
    ...
    if (const CItemBaseDupe *pDupeDef = CItemBaseDupe::GetDupeRef(uiDispID))
    {
...
bool CItem::SetBase( CItemBase * pItemDef )
{
    ...
    const auto pParentCont = dynamic_cast <CContainer*> (GetParent());
	int iWeightOld = 0;
...
void CItem::SetAmount(const word amount )
{
    ...
    if (const auto pParentCont = dynamic_cast<CContainer *>(GetParent()))
	{
		ASSERT( IsItemEquipped() || IsItemInContainer());
...
bool CItem::CanSendAmount() const noexcept
{
    ...
    if (const ITEMID_TYPE id = GetDispID(); id == ITEMID_WorldGem) // it can be a natural resource worldgem bit (used for lumberjacking, mining...)
        return false;
    return true;
}
...
void CItem::r_Write( CScript & s )
{
    ...
    if (const ITEMID_TYPE iDispID = GetDispID(); iDispID != GetID())	// the item is flipped.
		s.WriteKeyStr("DISPID", g_Cfg.ResourceGetName(CResourceID(RES_ITEMDEF, iDispID)));

    if (const int iAmount = GetAmount(); iAmount != 1)
	...
	if (const CObjBase* pCont = GetContainer())
	{
		if ( pCont->IsChar() )
		{
            if (const LAYER_TYPE iEqLayer = GetEquipLayer(); iEqLayer >= LAYER_HORSE )
		...
		if ( pCont->IsItem() )
		{
            ...
            if ( const uchar uiGridIdx = GetContainedGridIndex() )
...
bool CItem::LoadSetContainer(const CUID& uidCont, LAYER_TYPE layer )
{
	...
	if ( pObjCont->IsItem() )
	{
        ...
        if (const auto pCont = dynamic_cast<CItemContainer *>(pObjCont); pCont != nullptr )
		{
	...
	{
        if (const auto pChar = dynamic_cast<CChar *>(pObjCont); pChar != nullptr )
		{
			// equip the item
			const CItemBase * pItemDef = Item_GetDef();
			ASSERT(pItemDef);
...
bool CItem::r_WriteVal( lpctstr ptcKey, CSString & sVal, CTextConsole * pSrc, const bool fNoCallParent, const bool fNoCallChildren )
{
	...
	switch ( index )
	{
		...
		case IC_SUMMONING:
			{
            if (const CVarDefCont *pVar = GetDefKey(ptcKey, true))
					sVal = pVar->GetValStr();
				else
					sVal.Clear();
			}
		...
		case IC_AMOUNT:
        {
            if (const auto pSpawn = static_cast<CCSpawn *>(GetComponent(COMP_SPAWN)))
            {
		...
		case IC_CONTP:
			{
            if (const CObjBase *pContainer = GetContainer();
                pContainer != nullptr && IsItem() && IsItemInContainer() && pContainer->IsValidUID() && pContainer->IsContainer() && pContainer->IsItem() )
		...
		case IC_DISPIDDEC:
			{
	 			...
	 			if ( IsType(IT_COIN)) // Fix money piles
				{
                    ...
                    if (const ushort uiAmount = GetAmount(); uiAmount >= 2 )
					{
						if ( uiAmount < 6)
							iVal = iVal + 1;
						else
		...
		case IC_DUPEITEM:
			{
            if (const ITEMID_TYPE id = GetID(); id != GetDispID() )
					sVal.FormatHex( id );
				else
					sVal.SetValFalse();
			}
...
bool CItem::r_LoadVal( CScript & s ) // Load an item Script
{
    ...
    switch (FindTableSorted(s.GetKey(), sm_szLoadKeys, std::size(sm_szLoadKeys) - 1))
	{
		...
		case IC_ADDSPELL:
			...
			{
                if (const auto spell = static_cast<SPELL_TYPE>(ResGetIndex(s.GetArgVal())); AddSpellbookSpell(spell, false))
					return false;
                break;
			}
		case IC_AMOUNT:
		...
		case IC_CONTP:
			{
                ...
                if (CObjBase *pContainer = GetContainer();
                    pContainer != nullptr && IsItem() && IsItemInContainer() && pContainer->IsValidUID() && pContainer->IsContainer() && pContainer->IsItem() )
				{
                    const auto pCont = dynamic_cast <CItemContainer *> ( pContainer );
		...
		case IC_LINK:
			{
            if (const auto uidLink = static_cast<CUID>(s.GetArgDWVal()); static_cast<dword>(uidLink) == 0)
                {
                    m_uidLink.InitUID();
                }
                else
...
TRIGRET_TYPE CItem::OnTrigger( lpctstr pszTrigName, CScriptTriggerArgsPtr const& pScriptArgs, CTextConsole * pSrc )
{
    ...
    const auto iAction = static_cast<ITRIG_TYPE>(_iRunningTriggerId);
...
standard_order:
	// 1) Triggers installed on character, sensitive to actions on all items
    {
        ...
        if (const auto iCharAction = static_cast<CTRIG_TYPE>(FindTableSorted(ptcCharTrigName, CChar::sm_szTrigName, std::size(CChar::sm_szTrigName) - 1));
            (iCharAction > XTRIG_UNKNOWN) && IsTrigUsed(ptcCharTrigName))
        {
            if (CChar *pChar = pSrc->GetChar(); pChar != nullptr)
            {
	...
	if ( IsTrigUsed(pszTrigName) )
	{
		...
		EXC_SET_BLOCK("typedef");
		{
            ...
            auto pResourceLink = dynamic_cast <CResourceLink *>( g_Cfg.RegisteredResourceGetDef( CResourceID( RES_TYPEDEF, GetType() )));
...
void CItem::Update(const CClient * pClientExclude)
{
	...
	for (CClient* pClient = it.next(); pClient != nullptr; pClient = it.next())
	{
		...
		{
            const auto pCont = dynamic_cast <CItemContainer*> (GetParent());
			ASSERT(pCont);
...
void CItem::ConvertBolttoCloth()
{
    ...
    const word iOutAmount = GetAmount();
    const auto pCont = dynamic_cast <CItemContainer*> ( GetContainer() );
	Delete();
...
CItem *CItem::Weapon_FindRangedAmmo(const CResourceID& id)
{
    ...
    const auto pParent = dynamic_cast<CContainer *>(dynamic_cast<CObjBase *>(GetParent()));

    if (const CSString sAmmoCont = GetPropStr(COMP_PROPS_ITEMWEAPONRANGED, PROPIWEAPRNG_AMMOCONT, true, true); !sAmmoCont.IsEmpty())
	{
		// Search container using UID
		lpctstr  ptcAmmoCont = sAmmoCont.GetBuffer();
        auto pCont = dynamic_cast<CContainer *>(CUID::ItemFindFromUID(Exp_GetDWVal(ptcAmmoCont)));
		if ( pCont )
		{
...
int CItem::Use_LockPick( CChar * pCharSrc, const bool fTest, const bool fFail )
{
    ...
    if (const auto pCharTop = dynamic_cast<CChar *>(GetTopLevelObj()); pCharTop && pCharTop != pCharSrc )
	{
...
int CItem::OnTakeDamage(const int iDmg, CChar * pSrc, const DAMAGE_TYPE uType )
{
	...
	{
forcedamage:
        const auto pChar = dynamic_cast <CChar*> ( GetTopLevelObj());
		...
		if ( pChar && pChar != pSrc )
		{
			if (pChar->IsPriv(PRIV_DETAIL))
			{
				...
				*pszMsg = 0;
                if (m_itArmor.m_wHitsCur < m_itArmor.m_wHitsMax / 2)
				{
                    if (const int iPercent = Armor_GetRepairPercent(); pChar->Skill_GetAdjusted(SKILL_ARMSLORE) / 10 > iPercent)
...
bool CItem::IsResourceMatch( const CResourceID& rid, const dword dwArg ) const
{
    ...
    {
        const auto index = static_cast<IT_TYPE>(rid.GetResIndex());
        if (!IsType(index))
            return false;
...
bool CItem::_CanHoldTimer() const
{
	...
	if (const CObjBase *pCont = GetContainer(); pCont != nullptr)
	{
		return pCont->IsChar();
	}
...
bool CItem::_CanTick(const bool fParentGoingToSleep) const
{
    ...
    const auto pCont = dynamic_cast<const CObjBase*>(pParent);
    const bool fCharCont = pCont && pCont->IsChar();
    if (const bool fAllowContained = (HAS_FLAGS_STRICT(g_Cfg.m_uiItemTimers, ITEM_CANTIMER_IN_CONTAINER) || Can(CAN_I_TIMER_CONTAINED));
        fCharCont && fAllowContained)
	{
...
bool CItem::_OnTick()
{
	...
	if (!_IsSleeping())
	{
        if (!_CanTick(false))
		{
            if (const CSector *pSector = GetTopSector(); pSector && pSector->IsSleeping())
			{
    ...
    */
    if (const CCRET_TYPE iCompRet = CEntity::_OnTick(); iCompRet != CCRET_CONTINUE) // if return = CCRET_TRUE or CCRET_FALSE
    {
		...
		case IT_CORPSE:
			{
                ...
                if (const CChar *pSrc = m_uidLink.CharFind(); pSrc && pSrc->m_pPlayer )
				{
                    if (const CClient *pClient = pSrc->GetClientActive())
                    {
		...
		case IT_POTION:
			{
				...
				if ( (ResGetIndex(m_itPotion.m_Type) == SPELL_Explosion) && m_itPotion.m_ignited )
				{
					...
					{
						--m_itPotion.m_tick;
                        const auto pObj = static_cast<CObjBase*>(GetTopLevelObj());
						ASSERT(pObj);
						tchar* pszMsg = Str_GetTemp();

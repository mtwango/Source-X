...
void CItemStone::r_Write( CScript & s )
{
    ...
    for (const auto *pMember = static_cast<CStoneMember *>(GetContainerHead()); pMember != nullptr; pMember = pMember->GetNext())
	{
...
lpctstr CItemStone::GetAlignName() const
{
    ...
    const lpctstr sRes = g_ExprGlobals.mtEngineLockedReader()->m_VarDefs.GetKeyStr(tsDefname);
...
bool CItemStone::r_GetRef( lpctstr & ptcKey, CScriptObj * & pRef )
{
	...
	if ( !strnicmp("member.", ptcKey, 7) )
	{
        ...
        auto *pMember = static_cast <CStoneMember *>(GetContainerHead());
...
bool CItemStone::r_LoadVal( CScript & s ) // Load an item Script
{
	...
	switch ( FindTableSorted( s.GetKey(), sm_szLoadKeys, std::size(sm_szLoadKeys) - 1 ))
	{
		...
		case STC_MASTERUID:
			{
				if ( s.HasArgs() )
				{
                    const CUID pNewMASTERUID(s.GetArgDWVal());
                    ...
                    if ( CStoneMember *pMaster = GetMasterMember() )
					{
		...
		case STC_MEMBER:
            ...
            const std::optional<dword> dwUID = Str_ToU(Arg_ppCmd[0]);
            if (!dwUID.has_value())
            {
...
bool CItemStone::r_WriteVal(const lpctstr ptcKey, CSString & sVal, CTextConsole * pSrc, const bool fNoCallParent, const bool fNoCallChildren )
{
	...
	if ( !strnicmp("member.",ptcKey,7) )
	{
		...
		if ( !strnicmp("COUNT",pszCmd,5) )
		{
			...
			int i = 0;
            auto const * pMember = static_cast <CStoneMember *>(GetContainerHead());

			if ( *pszCmd )
			{
				SKIP_ARGSEP(pszCmd);
                const auto iPriv = static_cast<STONEPRIV_TYPE>(Exp_GetVal(pszCmd));
		...
		case STC_LOYALTO:
			{
				...
				{
                    if (CChar const * pLOYALTO = pMember->GetLoyalToUID().CharFind(); (pLOYALTO == nullptr) || (pLOYALTO == pCharSrc ))
					{
	...
	switch ( index )
	{
		...
		case ISV_DECLAREWAR:
			if ( s.HasArgs())
			{
				...
				{
                    if (auto *pNewEnemy = static_cast<CItemStone *>(pEnemyItem) )
...
CStoneMember * CItemStone::GetMasterMember() const
{
    ...
    for (auto *pMember = static_cast<CStoneMember *>(GetContainerHead()); pMember != nullptr; pMember = pMember->GetNext())
	{
...
CStoneMember * CItemStone::GetMember( const CObjBase * pObj ) const
{
	...
	if (!pObj)
		return nullptr;
    const CUID& otherUID = pObj->GetUID();
    for (auto *pMember = static_cast<CStoneMember *>(GetContainerHead()); pMember != nullptr; pMember = pMember->GetNext())
	{
...
bool CItemStone::NoMembers() const
{
    ...
    for (CStoneMember const *pMember = static_cast<CStoneMember *>(GetContainerHead()); pMember != nullptr; pMember = pMember->GetNext())
	{
...
CStoneMember * CItemStone::AddRecruit(const CChar * pChar, STONEPRIV_TYPE iPriv, const bool bFull)
{
	...
	tchar * z = Str_GetTemp();
    if (const CItemStone *pStone = pChar->Guild_Find(GetMemoryType()); pStone && pStone != this )
	{
...
int CItemStone::FixWeirdness()
{
	...
	bool fChanges = false;
    const auto *pMember = static_cast <CStoneMember *>(GetContainerHead());

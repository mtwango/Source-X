...
bool CChar::TeleportToObj(const int iType, tchar * pszArgs )
{
	...
	while ( dwCount )
	{
		...
		switch ( iType )
		{
			case 0:
			{
                if (const MATCH_TYPE match = Str_Match(pszArgs, pObj->GetName()); match != MATCH_VALID)
					continue;
			}
			break;
			case 1:	// char
			...
			case 3: // char id
			{
				if (!pObj->IsChar())
					continue;
                if (const CChar *pChar = static_cast<CChar *>(pObj); pChar->GetID() != iArg)
					continue;
			}
			break;
			case 4:	// item id
			{
				if (!pObj->IsItem())
					continue;
                if (const CItem *pItem = static_cast<CItem *>(pObj); pItem->GetID() != iArg)
					continue;
			}
			break;
		}
...
void CChar::AddGoldToPack( int iAmount, CItemContainer * pPack, const bool fForceNoStack )
{
	...
	while ( iAmount > 0 )
	{
        ...
        const word iGoldStack = static_cast<word>(minimum(iAmount, iMax));
...
void CChar::OnRemoveObj( CSObjContRec* pObRec )	// Override this = called when removed from list.
{
    ...
    if (const CCPropsItemEquippable *pItemBaseCCPItemEquippable = pItemBase->GetComponentProps<CCPropsItemEquippable>();
        pItemCCPItemEquippable || pItemBaseCCPItemEquippable)
    {
        ...
        if (pItem->IsTypeWeapon())
        {
            if (const CItem *pCursedMemory = LayerFind(LAYER_SPELL_Curse_Weapon))
...
void CChar::DropAll(CItemContainer * pCorpse, const uint64 uiAttr)
{
    ...
    if (CItemContainer *pPack = GetPack(); pPack != nullptr )
	{
		if ( pCorpse == nullptr )
		{
...
ANIM_TYPE CChar::GenerateAnimate( ANIM_TYPE action, const bool fTranslate, const bool fBackward, byte iFrameDelay, const byte iAnimLen )
{
	...
	{
        if (const CItem *pWeapon = m_uidWeapon.ItemFind(); pWeapon != nullptr && action == ANIM_ATTACK_WEAPON)
		{
			// action depends on weapon type (skill) and 2 Hand type.
            const LAYER_TYPE layer = pWeapon->Item_GetDef()->GetEquipLayer();
        ...
        if (!IsPlayableCharacter())
        {
            ...
            const CREID_TYPE dispID = GetDispID();
            ...
            if (fUseMobTypes)
            {
                ...
                if (const CUOMobTypesEntry *pMobtEntry = g_Install.m_mobtypes.GetEntry(dispID); pMobtEntry->m_uiType == MOBTE_QTY)
                {
            ...
            if (mobTypesRow.m_uiFlags & ATFLAG_UseUopAnimation)
            {
                ...
                switch (action)
                {
                    ...
                    case ANIM_WALK_ARM:
                        //Creatures without walk animation
                        {
                            ...
                            if (dispID == CREID_TUNNEL_SPIRIT_BODY || dispID == CREID_CHARYBDIS ||
                                (dispID >= CREID_PUMPKIN_DEMON && dispID <= CREID_CLOCKWORK_EXODUS) || dispID == CREID_JACK_IN_THE_BOX)
                            {
                                return ANIM_UOP_WALK_COMBAT;
                            }
                            if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_LITHOS)
                            {
                                return ANIM_UOP_STAND;
                            }
                            if (dispID == CREID_TS_TENTACLE || dispID == CREID_CHARYBDIS_TENTACLE)
                            {
                    ...
                    case ANIM_WALK_WAR:
                        ...
                        {
                            //Creatures without walk combat animation
                            if (dispID == CREID_PARROT_BIRD || dispID == CREID_TURKEY_GIANT || dispID == CREID_HELLHOUND_ANCIENT ||
                                dispID == CREID_GIANT_GORILLA || dispID == CREID_HORSE_PALOMINO)
                            {
                                return ANIM_UOP_WALK;
                            }
                            if (dispID == CREID_PHOENIX)
                            {
                                return ANIM_UOP_FLY;
                            }
                            if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_TS_TENTACLE || dispID == CREID_CHARYBDIS_TENTACLE || dispID == CREID_LITHOS)
                            {
                                return ANIM_UOP_STAND_COMBAT;
                            }
                            return ANIM_UOP_WALK_COMBAT;
                    ...
                    case ANIM_RUN_ARMED:
                        //If the creature can fly we can use flying action for running
                        {
                            ...
                            if (dispID == CREID_TUNNEL_SPIRIT_BODY || dispID == CREID_CHARYBDIS ||
                                (dispID >= CREID_PUMPKIN_DEMON && dispID <= CREID_CLOCKWORK_EXODUS))
                            {
                                return ANIM_UOP_WALK_COMBAT;
                            }
                            if (dispID == CREID_ANIMATED_WEAPON)
                            {
                                return ANIM_UOP_STAND;
                            }
                            if (dispID == CREID_TS_TENTACLE || dispID == CREID_CHARYBDIS_TENTACLE)
                    ...
                    case ANIM_STAND:
                        ...
                        {
                            //Creatures without stand anim
                            if (dispID == CREID_TUNNEL_SPIRIT_BODY || dispID == CREID_TS_TENTACLE || dispID == CREID_CHARYBDIS ||
                                dispID == CREID_CHARYBDIS_TENTACLE || dispID == CREID_CLOCKWORK_EXODUS || dispID == CREID_JACK_IN_THE_BOX)
                            {
                                return ANIM_UOP_STAND_COMBAT;
                            }
                            return ANIM_UOP_STAND;
                    ...
                    case ANIM_EAT:
                    {
                        ...
                        if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_TUNNEL_SPIRIT_BODY || dispID == CREID_TS_TENTACLE ||
                            (dispID >= CREID_CHARYBDIS && dispID <= CREID_CLOCKWORK_EXODUS) || dispID == CREID_JACK_IN_THE_BOX)
                        {
                            return ANIM_UOP_SPECIAL;
                        }
                    ...
                    case ANIM_SALUTE:
                    {
                        ...
                        if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_TS_TENTACLE || (dispID >= CREID_CHARYBDIS && dispID <= CREID_CLOCKWORK_EXODUS))
                        {
                            return ANIM_UOP_SPECIAL;
                        }
                        ...
                        if (dispID == CREID_CLOCKWORK_EXODUS)
                        {
                            switch (g_Rand.GetVal(2))
                            {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                    return ANIM_UOP_CAST_2;
                                    break;
                            }
                        }
                        //Creatures with cast action
                        else if (dispID == CREID_DRAGON_CRIMSON || dispID == CREID_DRAGON_PLATINUM || dispID == CREID_GRUBBER || dispID == CREID_STONE_FORM ||
                                 dispID == CREID_ABYSSAL_INFERNAL || (dispID >= CREID_ELEM_LAVA && dispID <= CREID_GARG_UNDEAD) ||
                                 dispID == CREID_GOBLIN_GREEN_MAGE || dispID == CREID_MEDUSA || dispID == CREID_SLASHER_VEILS ||
                                 dispID == CREID_DRAGON_STYGIAN || dispID == CREID_LICH_PRIMEVAL || dispID == CREID_SCALIS ||
                                 dispID == CREID_HELLHOUND_ANCIENT || dispID == CREID_MINOTAUR_LORD || dispID == CREID_PUMPKIN_FRIGHT ||
                                 dispID == CREID_CLOCKWORK_EXODUS || dispID == CREID_MYRMIDEX_DRONE || dispID == CREID_MYRMIDEX_WARRIOR ||
                                 dispID == CREID_ZIPACTRIOTL || dispID == CREID_LASHER || dispID == CREID_WINDRUNNER || dispID == CREID_HYDROS ||
                                 dispID == CREID_JACK_IN_THE_BOX || (dispID >= CREID_LITHOS && dispID <= CREID_PYROS) || dispID == CREID_KHAL_ANKUR ||
                                 dispID == CREID_WAR_BOAR_MOUNT)
                        {
                            return ANIM_UOP_CAST_1;
                        }
                    ...
                    case ANIM_ATTACK_WRESTLE:
                        //No attack
                        {
                            ...
                            //No attack 2
                            if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_TS_TENTACLE || dispID == CREID_TURKEY_GIANT ||
                                dispID == CREID_HORSE_PALOMINO || dispID == CREID_BLOOD_FOX || dispID == CREID_KRUMPUS_IMP ||
                                (dispID >= CREID_CHARYBDIS_TENTACLE && dispID <= CREID_PUMPKIN_FRIGHT) ||
                                (dispID >= CREID_DOG_NEWFOUNDLAND && dispID <= CREID_DOG_ROTTWEILER))
                            {
                                return ANIM_UOP_ATTACK_1;
                            }
                            switch (g_Rand.GetVal(2))
                    ...
                    case ANIM_DIE_FORWARD:
                        //No die forward
                        {
                            //No die forward
                            if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_TUNNEL_SPIRIT_BODY || dispID == CREID_TS_TENTACLE ||
                                dispID == CREID_PARROT_BIRD || dispID == CREID_PHOENIX || dispID == CREID_TURKEY_GIANT || dispID == CREID_HELLHOUND_ANCIENT ||
                                (dispID >= CREID_CHARYBDIS && dispID <= CREID_PUMPKIN_FRIGHT) || dispID == CREID_GIANT_GORILLA || dispID == CREID_BLOOD_FOX ||
                                dispID == CREID_FROST_MITE || dispID == CREID_STRATOS || dispID == CREID_KRUMPUS_IMP ||
                                (dispID >= CREID_WAR_BOAR_MOUNT && dispID <= CREID_WAR_CAPYBARA_BABY) ||
                                (dispID >= CREID_RABBIT_DOOM_BABY && dispID <= CREID_DOG_ROTTWEILER))
                            {
                                return ANIM_UOP_DIE_BACKWARD;
                            }
                    ...
                    case ANIM_BLOCK:
                        //No Block
                        {
                            ...
                            if (dispID == CREID_ANIMATED_WEAPON || dispID == CREID_TS_TENTACLE || dispID == CREID_PARROT_BIRD || dispID == CREID_PHOENIX ||
                                dispID == CREID_CHARYBDIS_TENTACLE || dispID == CREID_PUMPKIN_FRIGHT || dispID == CREID_FROST_MITE)
                            {
                                return ANIM_UOP_GETHIT;
                            }
                            return ANIM_UOP_BLOCK;
                    ...
                    case ANIM_PILLAGE:
                        ...
                        {
                            //Creatures with Pillage
                            if (dispID == CREID_GARG_FIRE || dispID == CREID_DRAGON_CRIMSON || dispID == CREID_DRAGON_PLATINUM || dispID == CREID_TROGLODYTE ||
                                dispID == CREID_GRUBBER || dispID == CREID_BLOODWORM || dispID == CREID_VOLLEM || dispID == CREID_GOBLIN_GRAY ||
                                dispID == CREID_STONE_FORM || (dispID >= CREID_MADDENING_HORROR && dispID <= CREID_GOBLIN_GREEN_MAGE) ||
                                dispID == CREID_MEDUSA || dispID == CREID_SKREE || dispID == CREID_SPIDER_TRAPDOOR || dispID == CREID_FIRE_ANT ||
                                dispID == CREID_DREAM_WRAITH || dispID == CREID_SLASHER_VEILS || dispID == CREID_GARG_ENSLAVED ||
                                dispID == CREID_DRAGON_STYGIAN || dispID == CREID_RISING_COLOSSUS || dispID == CREID_LICH_PRIMEVAL || dispID == CREID_SCALIS ||
                                dispID == CREID_MINOTAUR_LORD || dispID == CREID_DRAGON_TURTLE || dispID == CREID_SAUROSAURUS ||
                                dispID == CREID_MYRMIDEX_LARVAE || dispID == CREID_DRAGON_TURTLE_HATCHL || dispID == CREID_MYRMIDEX_DRONE ||
                                dispID == CREID_MYRMIDEX_WARRIOR || dispID == CREID_ZIPACTRIOTL || dispID == CREID_KHAL_ANKUR)
                            {
                    ...
                    case ANIM_SUMMON:
                    {
                        if (dispID == CREID_WILD_TIGER || dispID == CREID_TIGER_ETHEREAL ||
                            (dispID >= CREID_GIANT_GORILLA && dispID <= CREID_TYRANNOSAURUS_REX) || dispID == CREID_LASHER || dispID == CREID_HORSE_PALOMINO ||
                            (dispID >= CREID_TRICERATOPS && dispID <= CREID_FROST_MITE) || dispID == CREID_LION || dispID == CREID_HYDROS ||
                            (dispID >= CREID_LITHOS && dispID <= CREID_PYROS) || dispID == CREID_TIGER_UNDEAD ||
                            (dispID >= CREID_CRAB_COCONUT_MOUNT && dispID <= CREID_CRAB_COCONUT_LARGE) ||
                            (dispID >= CREID_WAR_BOAR_MOUNT && dispID <= CREID_WAR_CAPYBARA_BABY) ||
                            (dispID >= CREID_RABBIT_DOOM_BABY && dispID <= CREID_DOG_ROTTWEILER))
                        {
                            return ANIM_UOP_ROAR;
                        ...
                        if (dispID == CREID_PHOENIX || dispID == CREID_TURKEY_GIANT || dispID == CREID_PUMPKIN_DEMON || dispID == CREID_PUMPKIN_FRIGHT)
                        {
                            return ANIM_UOP_PEACE_TO_COMBAT;
                        }
                        return ANIM_UOP_SPECIAL;
                     ...
                     (mobTypesRow.m_uiType == MOBTE_MONSTER && (mobTypesRow.m_uiFlags & ATFLAG_CalculateOffsetByLowGroup)))
            {
                ...
                switch (action)
                {
                    ...
                    case ANIM_SALUTE:
                    {
                        ...
                        //No Lie Down
                        if (dispID == CREID_CAT || dispID == CREID_PANTHER || dispID == CREID_COW_BW || dispID == CREID_DOG || dispID == CREID_WOLF_TIMBER ||
                            (dispID >= CREID_COW_BROWN && dispID <= CREID_BULL_BROWN_DK))
                        {
                            return ANIM_ANI_FIDGET1;
                        }
                        if (dispID == CREID_SQUIRREL)
                        {
                            return ANIM_ANI_FIDGET2;
                        }
            ...
            else
            {
                ...
                switch (action)
                {
                    ...
                    case ANIM_RUN_ARMED:
                        ...
                        {
                            //If the creature can fly we can use flying action for running
                            if ((mobTypesRow.m_uiFlags & ATFLAG_CanFlying) || (pCharDef->m_Anims & AFLAG_CUST_MON_FLY)) //Custom anims with Fly action
                            {
                                return ANIM_MON_FLY;
                            }
                            return ANIM_MON_WALK;
                    ...
                    case ANIM_EAT:
                    {
                        ...
                        if (dispID == CREID_EAGLE || dispID == CREID_DRAGON_CRIMSON || dispID == CREID_DRAGON_PLATINUM) //No Fidget
                        {
                            return ANIM_MON_FIDGET1;
                        }
                        return ANIM_MON_FIDGET2;
                    ...
                    case ANIM_ATTACK_XBOW: //return ANIM_MON_AttackXBow; //Always Empty except for standard ID 13, 15 and 16
                        ...
                        //Cast 2
                        if (dispID == CREID_TERA_MATRIARCH || dispID == CREID_OPHID_SHAMAN || dispID == CREID_GARGOYLE || dispID == CREID_DEMON ||
                            dispID == CREID_DEMON_SWORD || dispID == CREID_LICH || (pCharDef->m_Anims & AFLAG_CUST_MON_CAST2)) //Custom Anim with Cast2 action
                        {
                            if (castActions == 0)
                            {
                        ...
                        else //No cast Actions
                        {
                            ...
                            if (dispID == CREID_SHADOW_LORD || (pCharDef->m_Anims & AFLAG_CUST_MON_NO_ATTACK1)) //Custom Anim without Attack1
                            {
                                attackActions--;
                            }
                    ...
                    case ANIM_GET_HIT:
                        ...
                        if (pCharDef->m_Anims & AFLAG_CUST_MON_ATTACKSFROM9) //Custom Anims with GetHit in Land/TakeOff action
                        {
                            return ANIM_MON_LAND;
                        }
                        if (pCharDef->m_Anims & AFLAG_CUST_MON_NO_GETHIT) //Cutsom Anims without GetHit
                        {
                    ...
                    case ANIM_DIE_BACK:
                        ...
                        {
                            //Die while flying animation
                            if ((mobTypesRow.m_uiFlags & ATFLAG_StompAction) && (mobTypesRow.m_uiFlags & ATFLAG_CanFlying) &&
                                (mobTypesRow.m_uiFlags & ATFLAG_Use10IfHittedWhileFlying) && IsStatFlag(STATF_FLY | STATF_HOVERING))
                            {
                                return ANIM_MON_DIE_FLIGHT;
                            }
                            return ANIM_MON_DIE1;
                    ...
                    case ANIM_DIE_FORWARD:
                        //Die while flying animation
                        {
                            ...
                            if (pCharDef->m_Anims & AFLAG_CUST_MON_NO_DIE2) //Custom Anims without Die2
                            {
                                return ANIM_MON_DIE1;
                            }
                            if (dispID == CREID_SHADOW_LORD) //No Die2
                            {
                                return ANIM_MON_DIE1;
                            }
                            return ANIM_MON_DIE2;
                    ...
                    case ANIM_THROW:
                    {
                        ...
                        if (pCharDef->m_Anims & AFLAG_CUST_MON_NO_ATTACK1) //Custom anims without Attack1
                        {
                            return ANIM_MON_STAND;
                        }
                        return ANIM_MON_ATTACK1;
                    ...
                    case ANIM_PILLAGE:
                    {
                        ...
                        if ((pCharDef->m_Anims & AFLAG_CUST_MON_NO_PILLAGE) &&
                            (!(pCharDef->m_Anims & AFLAG_CUST_MON_NO_FIDGET1))) //Custom Anims with No Pillage but Fidget1
                        {
                            return ANIM_MON_FIDGET1;
                        }
                        if ((pCharDef->m_Anims & AFLAG_CUST_MON_NO_PILLAGE) &&
                            (!(pCharDef->m_Anims & AFLAG_CUST_MON_NO_FIDGET2))) //Custom Anims with No Pillage but Fidget2
                        {
                            return ANIM_MON_FIDGET2;
                        }
                        if (dispID == CREID_BIRD || dispID == CREID_EAGLE || dispID == CREID_GAZER || dispID == CREID_SHADOW_LORD) //No Pillage
                        {
                            return ANIM_MON_FIDGET1;
                        }
                        if (dispID == CREID_CORPSER) //No Pillage
                        {
                            return ANIM_MON_FIDGET2;
                        }
                        return ANIM_MON_PILLAGE;
                    ...
                    case ANIM_SUMMON:
                    {
                        ...
                        if ((mobTypesRow.m_uiFlags & ATFLAG_CanFlying) &&
                            !(pCharDef->m_Anims & AFLAG_CUST_MON_FLY)) //Standard flying anims with Land/TakeOff action
                        {
                            return ANIM_MON_LAND;
                        }
                    ...
                    case ANIM_GETHIT_AIR:
                    {
                        ...
                        if (mobTypesRow.m_uiFlags & ATFLAG_Use10IfHittedWhileFlying)
                        {
                            return ANIM_MON_GETHIT;
                        }
                        return ANIM_MON_DIE_FLIGHT;
...
bool CChar::UpdateAnimate(ANIM_TYPE action, const bool fTranslate, const bool fBackward , const byte iFrameDelay , const byte iAnimLen)
{
	...
	for (const CClient * pClient = it.next(); pClient != nullptr; pClient = it.next())
	{
        ...
        if (const CNetState *state = pClient->GetNetState(); state->isClientEnhanced() || state->isClientKR())
			cmdnew->send(pClient);
...
void CChar::UpdateMove( const CPointMap & ptOld, const CClient * pExcludeClient, const bool bFull )
{
	...
	for ( CClient* pClient = it.next(); pClient != nullptr; pClient = it.next() )
	{
        ...
        const bool fCouldSee = (ptOld.GetDistSight(pChar->GetTopPoint()) <= pChar->GetVisualRange());
...
int CChar::ItemPickup(CItem * pItem, word amount)
{
	...
	if( IsClientActive() )
	{
        ...
        if (const auto pItemCont = dynamic_cast<CItem *>(pItemParent); pItemCont != nullptr )
		{
			...
			if ( pCharTop != nullptr )
			{
                const CItemContainer * pBank = pCharTop->GetBank();
                if (const bool fItemContIsInsideBankBox = pBank->IsItemInside(pItemCont); fItemContIsInsideBankBox && (pBank->m_itEqBankBox.m_pntOpen != ptTop))
					return -1;
			}
	...
	{
        if (const int iItemWeight = pItem->GetWeight(amount); (GetWeightLoadPercent(GetTotalWeight() + iItemWeight)) > g_Cfg.m_iDragWeightMax)
		{
	...
	if (( trigger == ITRIG_PICKUP_GROUND ) && IsSetEF( EF_ItemStackDrop ))
	{
        ...
        const char iStackMaxZ = GetTopZ() + 16;
...
bool CChar::ItemDrop( CItem * pItem, const CPointMap & pt )
{
	...
	{
        const char iItemHeight = pItem->GetHeight();
        const CServerMapBlockingState block( CAN_C_WALK, pt.m_z, pt.m_z, pt.m_z, maximum(iItemHeight,1) );
		...
		if (pStack != nullptr) //If there nothing  on the ground, drop the item normally and flip it if it's possible
		{
			for (uint i = 0;; ++i)
			{
                ...
                if (const char iStackZ = pStack->GetTopZ(); iStackZ < pt.m_z || iStackZ > iStackMaxZ )
					continue;
    ...
    if (const CItemBase *pItemDef = pItem->Item_GetDef();
        ( g_Cfg.m_fFlipDroppedItems && pItem->Can(CAN_I_FLIP)) && pItem->IsMovableType() && !pItemDef->IsStackableType())
...
bool CChar::ItemEquip( CItem * pItem, const CChar * pCharMsg, const bool fFromDClick )
{
    ...
    const LAYER_TYPE layer = CanEquipLayer(pItem, LAYER_QTY, pCharMsg, false);
    ...
    if (CItemBase::IsVisibleLayer(layer))	// visible layer ?
    {
        ...
        if (const CVarDefCont *pVar = GetDefKey("EQUIPSOUND", true) )
        {
    ...
    if (const CCPropsItemEquippable *pItemBaseCCPItemEquippable = pItemBase->GetComponentProps<CCPropsItemEquippable>(); pItemCCPItemEquippable || pItemBaseCCPItemEquippable)
    {
        // Leave the stat bonuses signed, since they can be used also as a malus (negative sign)
        Stat_AddMod(STAT_STR,       + pItem->GetPropNum(pItemCCPItemEquippable, PROPIEQUIP_BONUSSTR, pItemBaseCCPItemEquippable));
        Stat_AddMod(STAT_DEX,       + pItem->GetPropNum(pItemCCPItemEquippable, PROPIEQUIP_BONUSDEX, pItemBaseCCPItemEquippable));
...
void CChar::EatAnim(CItem* pItem, const ushort uiQty)
{
    ...
    if (const EMOTEFLAGS_TYPE eFlag = (IsPlayer() ? EMOTEF_HIDE_EAT_PLAYER : EMOTEF_HIDE_EAT_NPC); !IsSetEmoteFlag(eFlag))
    {
        tchar* pszMsg = Str_GetTemp();
...
bool CChar::Reveal( uint64 iFlags )
{
    ...
    const CClient * pClient = IsClientActive() ? GetClientActive() : nullptr;
...
CItem* CChar::Horse_GetValidMountItem()
{
    ...
    if (const CChar *pOwner = NPC_PetGetOwner(); !pMountItem && pOwner)
	{
		...
		if (pMountItem)
		{
			...
			{
                if (const CChar *pLinkedChar = pMountItem->m_uidLink.CharFind(); pLinkedChar && pLinkedChar->IsStatFlag(STATF_RIDDEN))
				{
	...
	else if (pMountItem)
    {
        ...
        {
            if (CItem *pRiderMountItem = pRider->LayerFind(LAYER_HORSE))
            {
...
ITEMID_TYPE CChar::Horse_GetMountItemID() const
{
    ...
    const lpctstr ptcMemoryID = g_ExprGlobals.mtEngineLockedReader()->m_VarDefs.GetKeyStr(ptcMountID);			// get the mount item defname from the mount_0x** defname

    const CResourceID memoryRid = g_Cfg.ResourceGetID(RES_ITEMDEF, ptcMemoryID);
...
bool CChar::Horse_UnMount()
{
	...
	{
        if (CItem *pShip = pMountItem->m_uidLink.ItemFind())
...
bool CChar::SetPoison(const int iSkill, const int iHits, const CChar * pCharSrc )
{
    ...
    if (const CClient *pClient = GetClientActive(); pClient && IsSetOF(OF_Buffs) )
	{
...
CChar::DeathRequestResult CChar::Death()
{
	...
	if ( m_pNPC )
	{
		...
		{
            if (CChar *pCRider = Horse_GetMountChar())
...
bool CChar::OnFreezeCheck() const
{
	...
	if ( m_pPlayer )
	{
		...
		{
            if ( const CSpellDef *pSpellDef = g_Cfg.GetSpellDef(m_atMagery.m_iSpell) )
			{
...
CRegion * CChar::CanMoveWalkTo( CPointMap & ptDst, const bool fCheckChars, const bool fCheckOnly, const DIR_TYPE dir, const bool fPathFinding )
{
    ...
    if (!IsPriv(PRIV_GM))
    {
        if (const CClient *pClient = GetClientActive(); pClient && pClient->m_pHouseDesign)
        {
	...
	if ( !fCheckOnly )
	{
        ...
        if (iWeight < iMaxWeight)
		{
            ...
            const CVarDefCont * pVal = GetKey("OVERRIDE.RUNNINGPENALTY", true);
...
TRIGRET_TYPE CChar::CheckLocationEffects(const bool fStanding)
{
    ...
    if (!fStanding)
    {
        if (const SKILL_TYPE iSkillActive = Skill_GetActive(); g_Cfg.IsSkillFlag(iSkillActive, SKF_IMMOBILE))
        {
            Skill_Fail(false);
        }
        ...
        if (thread_local uint _uiRecursingStep = 0; _uiRecursingStep >= _kuiRecursingStepLimit)
        {
        ...
        else
        {
            if (m_pArea && IsTrigUsed(TRIGGER_STEP)) //Check if m_pArea is exists because it may be invalid if it try to walk while multi removing?
            {
                ...
                if (CRegion *pRoom = GetTopPoint().GetRegion(REGION_TYPE_ROOM); pRoom && pRoom->OnRegionTrigger(this, RTRIG_STEP) == TRIGRET_RET_TRUE)
                {
                    _uiRecursingStep -= 1;
                    return TRIGRET_RET_FALSE;
                }
    ...
    for (;;)
    {
        ...
        const int zdiff = pItem->GetTopZ() - GetTopZ();
        ...
        if (IsTrigUsed(TRIGGER_STEP) || IsTrigUsed(TRIGGER_ITEMSTEP))
        {
            ...
            else
            {
                ...
                const TRIGRET_TYPE iRet = pItem->OnTrigger(ITRIG_STEP, pScriptArgs, this);
                _uiRecursingItemStep -= 1;
        ...
        switch (pItem->GetType())
        {
            ...
            case IT_FIRE:
            {
                ...
                if (const int iDmg = OnTakeDamage(g_Cfg.GetSpellEffect(SPELL_Fire_Field, iSkillLevel), nullptr, DAMAGE_FIRE | DAMAGE_GENERAL, 0, 100, 0, 0, 0); iDmg > 0)
                {
	...
	if ( m_pNPC )
	{
		...
		if ( m_pNPC->m_Brain == NPCBRAIN_GUARD )
		{
            ...
            if (const auto pArea = dynamic_cast<const CRegionWorld *>(pTeleport->_ptDst.GetRegion(REGION_TYPE_MULTI | REGION_TYPE_AREA)); !pArea || (!pArea->IsGuarded() && !IsSetOF(OF_GuardOutsideGuardedArea)) )
				return TRIGRET_RET_FALSE;
		}
		if ( Noto_IsCriminal() )
		{
			// wont teleport to guarded areas.
            if (const auto pArea = dynamic_cast<const CRegionWorld *>(pTeleport->_ptDst.GetRegion(REGION_TYPE_MULTI | REGION_TYPE_AREA)); !pArea || pArea->IsGuarded() )
				return TRIGRET_RET_FALSE;
		}
	}
...
bool CChar::MoveToChar(const CPointMap& pt, const bool fStanding, const bool fCheckLocationEffects, const bool fForceFix, const bool fAllowReject)
{
    ...
    if (const CClient *pClient = GetClientActive(); m_pPlayer && !pClient )	// moving a logged out client ! This happens on startup and when moving on a ship and when moving around disconnected characters by other means.
	{
	...
	ASSERT(pNewSector);
    const bool fSectorChanged = pNewSector->MoveCharToSector(this);
...
bool CChar::MoveToValidSpot(const DIR_TYPE dir, const int iDist, const int iDistStart, const bool fFromShip)
{
    ...
    const char startZ = pt.m_z;

    const uint64 uiCan = GetCanMoveFlags(GetCanFlags(), true);	// CAN_C_SWIM
	for ( int i=0; i<iDist; ++i )
	{
		if ( pt.IsValidPoint() )
		{
			// Don't allow boarding of other ships (they may be locked)
            if ( pt.GetRegion(REGION_TYPE_SHIP))
			{
				pt.Move( dir );
				continue;
			}
...
bool CChar::SetPrivLevel(const CTextConsole * pSrc, const lpctstr pszFlags)
{
    ...
    const PLEVEL_TYPE PrivLevel = CAccount::GetPrivLevelText(pszFlags);
...
void CChar::OnTickStatusUpdate()
{
    ...
    if (const int64 iTimeDiff = iTimeCur - _iTimeLastHitsUpdate; g_Cfg.m_iHitsUpdateRate && ( iTimeDiff >= g_Cfg.m_iHitsUpdateRate ) )
	{
...
void CChar::OnTickFood(const ushort uiVal, const int HitsHungerLoss)
{
    ...
    const short iFoodLevel = Food_GetLevelPercent();
	if ( iFoodLevel > 40 )
		return;
	if ( HitsHungerLoss <= 0 || IsStatFlag(STATF_SLEEPING) )
		return;

    const bool fPet = IsStatFlag(STATF_PET);
    const lpctstr pszMsgLevel = Food_GetLevelMessage(fPet, false);
    ...
    if ( CItem *pMountItem = Horse_GetValidMountItem() )
...
bool CChar::IsTickableEvenIfDisconnected() const
{
    ...
    if (const bool fShouldCheck = !g_Serv.IsStartupLoadingScripts(); fRidden && fShouldCheck)
    {
        ...
        if (const CChar *pRider = !pLinkedItem ? nullptr : pLinkedItem->m_uidLink.CharFind(); pRider && pLinkedItem->IsType(IT_EQ_HORSE))
        {
            ...
            if (!IsStatFlag(STATF_PET))
            {
                ...
                if (const bool fIsGm = pRider && pRider->IsPriv(PRIV_GM); !fIsGm)
...
//#endif
	...
	*/
    if (const CCRET_TYPE iCompRet = CEntity::_OnTick(); iCompRet != CCRET_CONTINUE) // if return != CCRET_TRUE
	{
